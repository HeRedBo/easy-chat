...
type (
	...
	GroupRequests struct {
		...
		ReqMsg        sql.NullString `db:"req_msg"`         // 招呼内容
		ReqTime       sql.NullTime   `db:"req_time"`        // 请求时间
		JoinSource    sql.NullInt64  `db:"join_source"`     // 添加来源
		InviterUserId sql.NullString `db:"inviter_user_id"` // 要求用户ID
		HandleUserId  sql.NullString `db:"handle_user_id"`  // 入群处理人ID
