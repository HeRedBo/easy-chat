...
type conversationsModel interface {
	...
	Upsert(ctx context.Context, data *Conversations) (*mongo.UpdateResult, error)
...
func (m *defaultConversationsModel) Upsert(ctx context.Context, data *Conversations) (*mongo.UpdateResult, error)  {
	...
	data.UpdateAt = time.Now()
	res, err := m.conn.ReplaceOne(ctx, bson.M{"_id": data.ID}, data,options.Replace().SetUpsert(true))
	return res, err
}
