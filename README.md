...
> 基于 Go-Zero 微服务框架构建的企业级即时通讯解决方案，深度实践云原生架构与 DevOps 最佳实践
...
| 层级 | 技术选型 | 说明 |
|------|----------|------|
| **开发框架** | Go-Zero v1.10 | 微服务 RPC 框架，内置服务治理 |
...
- **自动熔断降级**：基于 go-zero 内置的熔断器实现服务自我保护
- **自适应限流**：智能流量控制，防止服务雪崩
- **分布式事务**：通过消息队列实现最终一致性
...
make -f deploy/make/build.mk build SVR=user MOD=rpc

# 或使用脚本启动
./scripts/start-users-api.sh
./scripts/start-users-rpc.sh
```
...
| 消息时序一致性 | 基于 Snowflake 的分布式 ID + 时间戳排序 |
| 未读消息计数 | Redis 原子计数 + 定时持久化 |
| 多端消息同步 | 消息序号机制 + 增量同步策略 |
| 消息可靠性 | 消息确认机制 + 重试队列 + 死信处理 |
